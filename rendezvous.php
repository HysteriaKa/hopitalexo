...
?>
    ...
    $sql = ("SELECT appointments.id, lastname, firstname, phone, DATE_FORMAT(dateHour, '%Y-%m-%d') AS daterdv, DATE_FORMAT(dateHour, '%H:%i:%s') AS timerdv  
    FROM appointments  JOIN patients ON patients.id = appointments.idPatients WHERE appointments.id=:id");
    $req1 = $bdd->prepare($sql);
    $req1->execute(array('id' => $getid));
    $donnees = $req1->fetch();
    ?>
            ...
            <form method="POST">
                <tbody>
                    <tr>
                        ...
                        <td><label class="pr-3" for=""><?php echo $donnees['daterdv']; ?></label><input type="date"
                                name="date" value="<?php echo $donnees['daterdv']; ?>" /></td>
                        <td><label class="pr-3" for=""><?php echo $donnees['timerdv']; ?></label><input type="time"
                                name="time" value="<?php echo $donnees['timerdv']; ?>" /></td>
